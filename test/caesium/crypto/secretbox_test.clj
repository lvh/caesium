...
(deftest secretbox-kat-test
  (testing "Implementation handles several KATs; decrypt & encrypt roundtrip"
    (are [secret-key nonce plaintext ciphertext] (and (u/array-eq (encrypt secret-key nonce plaintext)
                                                                  ciphertext)
                                                      (u/array-eq (decrypt secret-key nonce ciphertext)
                                                                  plaintext))
      secret-key (int->nonce 0) message (u/unhexify "cb97a2f8b60ea0cafd933dd497c0eddc1a7b8224b8c3d147393a06664a289eb8914c009137895b290f")
      secret-key (int->nonce 1) message (u/unhexify "74f47d0db1fc9d22265d85218cd546e1924ef845d27696ba971614282f0a6647a23b481a91dd20399a")))
  (testing "Decrypting bogus ciphertexts causes exceptions"
    (are [secret-key nonce ciphertext]
        (thrown-with-msg? RuntimeException #"Decryption failed. Ciphertext failed verification"
                          (decrypt secret-key nonce ciphertext))
      secret-key (int->nonce 1) (u/unhexify "74f47d0db1fc9d22265d85218cd546e1924ef845d27696ba971614282f0a6647a23b481a91dd203990"))))

(deftest int->nonce-test
  (testing "Turning numbers into nonces works"
    (are [n nonce] (u/array-eq (int->nonce n) nonce)
      0 (byte-array 24)
      0M (byte-array 24)
